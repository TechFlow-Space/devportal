...
* Deploy example java SCORE using goloop CLI binary.

## Requirements

* [Download and Install Docker](https://docs.docker.com/get-docker/)
* You need to install Golang. Visit [https://golang.org/doc/install](https://golang.org/doc/install)
* You need to install OpenJDK 11. Visit [OpenJDK.net](http://openjdk.java.net/) for prebuilt binaries. Or you can install a proper OpenJDK package from your OS vendors.
* You need to install Gradle. Visit [https://gradle.org/](https://gradle.org/)
* It's easier to manage java and gradle version using Sdkman. Visit [https://sdkman.io/install](https://sdkman.io/install)
* Usage of Sdkman.  visit [https://sdkman.io/usage](https://sdkman.io/usage)
###Install Golang in Linux
```text
sudo apt-get install golang
```
### View available Java sources
```
sdk list java
```
### Install AdoptOpenJDK java
```
sdk install 11.0.11.hs-adpt
```
### View Gradle versions
```
sdk list gradle
```
### Install gradle
...
Visit [https://github.com/icon-project/gochain-local](https://github.com/icon-project/gochain-local)
```text
$ git clone git@github.com:icon-project/gochain-local.git
$ GOCHAIN_LOCAL_ROOT=/path/to/gochain-local
$ sudo chown -R your_user:your_group /path/to/gochain-local
```

Then, you need to checkout the `goloop` repository for building docker image and new javaee-api.jar.
Visit [https://github.com/icon-project/goloop](https://github.com/icon-project/goloop)

```text
$ git clone git@github.com:icon-project/goloop.git
$ GOLOOP_ROOT=/path/to/goloop
$ sudo chown -R your_user:your_group /path/to/goloop
```
...
## Step 3. Build javaee-api.jar(java Icon Score Execution Environment) for local
...
$ ./gradlew api:build
```
## Step 4. Create your example score
```text
# Create a project root dir for your java score example.
$ JAVA_SCORE_EXAMPLES_ROOT=/path/to/project_root
mkdir -p hello-world/src/main/java/com/iconloop/score/example/
```
...
dependencies {
    compile files('api-0.9.0-SNAPSHOT.jar')
...
#### copy jar file from step 3
```shell script
$ cp ${GOLOOP_ROOT}/api/build/libs/api-0.8.7-SNAPSHOT.jar ${JAVA_SCORE_EXAMPLES_ROOT}/hello-world/api-0.8.7-SNAPSHOT.jar

```
### Step 4. Build example score for local
```shell script
cd ${JAVA_SCORE_EXAMPLES_ROOT}
./gradlew build
./gradlew optimizedJar
cp ./build/libs/hello-world-0.1.0-optimized.jar ${GOCHAIN_LOCAL_ROOT}
```
